...
def train_network():
    ...
    val_imgs, val_labels = create_test_data(val_list, labels, VAL_PATH, positive_box_threshold=0.7, negative_box_threshold=0.3)
    ...
    for epoch in range(NUM_EPOCHS):
        print("\n\n\nEPOCH: %d" % epoch)
        dataset = create_batch_list(train_list, labels, positive_box_threshold=0.7, negative_box_threshold=0.3)
        for (x, y) in dataset:
